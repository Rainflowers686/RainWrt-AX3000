...
tracked_hits="$(git grep -Il -E "$private_pattern" -- . ':!docs/PUBLIC_RELEASE_AUDIT.md' 2>/dev/null | wc -l)"
history_hits="$(git log --all-match -G "$private_pattern" --format='%H' "$base..HEAD" -- . | sort -u | wc -l)"
