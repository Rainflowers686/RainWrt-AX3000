...
wifi="$(ubus call network.wireless status | jq '[to_entries[] | {name:.key, up:.value.up, disabled:(.value.disabled // false), pending:(.value.pending // false), band:(.value.config.band // "")}]')"
...
jq -n --arg board "$board" --arg kernel "$kernel" --argjson identity "$identity" --argjson release "$release" \
 ...
 --arg phy "$phy" --arg rproc "$rproc" --argjson wifi "$wifi" --arg bands "$bands" --arg bdf "$bdf" \
 ...
 ubi:$ubi,ubi_mtd:$ubi_mtd,mounts:$mounts,fatal:$fatal,phy:$phy,rproc:$rproc,wifi:$wifi,bands:$bands,bdf:$bdf,
